...
export function createSkillRepository(db: AppDatabase): SkillRepository {
  async function insertFiles(
    skillId: string,
    files: NonNullable<
      Parameters<SkillRepository["create"]>[0]["skillFiles"]
    >,
  ) {
    ...
    for (const file of files) {
      await db.insert(skillFiles).values({        id: (file as { id?: string }).id ?? Crypto.randomUUID(),
        skillId,
        path: file.path,
  ...
  return {
    async create(input) {
      ...
      await insertFiles(id, input.skillFiles ?? []);
