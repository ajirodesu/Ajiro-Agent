...
export function getMirrorRoot(session: ExternalFolderSession): Directory {
  const safeName =
    session.displayName
...
function resolveEntry(root: Directory, relativePath: string): Directory | File | null {
  ...
  for (let index = 0; index < parts.length; index += 1) {
    ...
    const childFile = new File(current, part);
    const childDir = new Directory(current, part);
  ...
  return {
    ...
    async rename(oldPath, newPath) {
      ...
      if (entry instanceof File) {
        entry.move(new File(parent, name), {
          overwrite: true,
        });
      } else {
        entry.move(new Directory(parent, name), {
          overwrite: true,
        });
      }
    },
  ...
  const walk = async (
    ...
    for (const entry of entries) {
      ...
      if (entry.kind === "directory") {
        ...
        const { bytes } = await service.readBytesFile(
          session,
          entry.path,
        );
